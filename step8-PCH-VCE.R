...
# run GIBBS sampler for all pch items
...
if(standardized){
  phn=fread(paste(workdir,"/std_pch.txt",sep=""),header=T,data.table=F)
}else{
  phn=fread(paste(workdir,"/pch.txt",sep=""),header=T,data.table=F)
}
items=colnames(phn)[substr(colnames(phn),1,3) == "pch"]

item.combn=data.frame(t(items))
rownames(item.combn)="MUV"
...
runGibbsSamplerMultiChain<-function(i.chain,i.combn,item.combn=item.combn,models=models,ped=ped,phn=phn,workdir=workdir,n.gibbs,n.burnin,n.thin,n.backup){
  
  setwd(workdir)
  # set up the data
  items=unlist(item.combn[i.combn,])
  # create a directory in which the estimation occurs
  estm.dir=paste(workdir,"/",rownames(item.combn)[i.combn],".chain",i.chain,sep="")
...
n.chain=32
system.time(re.gibbs<-mclapply(1:n.chain,i.combn=1,runGibbsSamplerMultiChain,item.combn=item.combn,
                               models=models,ped=ped,phn=phn,workdir=workdir,
                               n.gibbs=22000000,n.burnin=2000000,n.thin=1000,n.backup=100,
                               mc.cores=min(n.chain,100),mc.preschedule = F,mc.silent = T))
