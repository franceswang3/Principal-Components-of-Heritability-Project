...
if(standardized){
  ...
  workdir="/data3/Bert/Frances/July2021/SUPER-PCH-VCE/"
}
...
if(standardized){
  phn=fread(paste(workdir,"/std_super_pch.txt",sep=""),header=T,data.table=F)
}else{
  phn=fread(paste(workdir,"/super_pch.txt",sep=""),header=T,data.table=F)
}
items=colnames(phn)[substr(colnames(phn),1,3) == "PCH"]

item.combn=data.frame(item=items)
rownames(item.combn)=items
...
system.time(re.gibbs<-mclapply(seq(1,nrow(item.combn),1),runGibbsSampler,item.combn=item.combn,
                             ...
                             n.gibbs=1*550000,n.burnin=1*50000,n.thin=1*100,n.backup=100,
